...
public class TodoController {
    ...
    public ResponseTemplate<String> saveTodoList(@RequestBody @Valid TodoSaveRequest request, Principal principal){
    ...
    public ResponseTemplate<String> updateTodoCheckStatus(@PathVariable Long todoId, @RequestBody @Valid BoolDto boolDto
    ) {
    ...
    public ResponseTemplate<List<Todo>> getTodos() {
        List<Todo> todos = todoService.getTodos();

        return new ResponseTemplate<>(HttpStatus.OK, "모든 투두를 성공적으로 불러왔습니다. ", todos);
    }

    @DeleteMapping("/todo/{todoId}")
    public ResponseTemplate<Void> deleteEvaluation(@PathVariable Long todoId) {
        todoService.deleteTodo(todoId);
        return new ResponseTemplate<>(HttpStatus.OK, todoId + "번 투두가 성공적으로 삭제되었습니다.");
    }
}
