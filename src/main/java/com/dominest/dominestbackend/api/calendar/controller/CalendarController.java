...
public class CalendarController {
    ...
    public ResponseTemplate<List<Calendar>> getEventsByDate(@PathVariable("date") String dateString) {
        List<Calendar> events = calendarService.getByDate(dateString);
        return new ResponseTemplate<>(HttpStatus.OK, dateString +"의 일정을 성공적으로 가져왔습니다.", events);

    }

    @GetMapping("/list/{date}") // 월별 일정 내보내기
    public ResponseTemplate<List<CalendarMonthResponse>> getEventsByMonth(@PathVariable("date") String dateString) {
        List<CalendarMonthResponse> darList = calendarService.getByMonth(dateString);
        return new ResponseTemplate<>(HttpStatus.OK, dateString +"월의 일정을 성공적으로 가져왔습니다.", darList);
    }
